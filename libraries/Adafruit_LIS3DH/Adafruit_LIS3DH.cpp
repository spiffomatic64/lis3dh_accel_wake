...
bool Adafruit_LIS3DH::setupLowPowerWakeMode(uint8_t movementThreshold, int pin) {
	...
	//
	writeRegister8(LIS3DH_REG_INT1DUR, 1);
	...
	if (intPin >= 0) {
		...
		int1_cfg =  LIS3DH_INT1_CFG_ZHIE_ZUPE | LIS3DH_INT1_CFG_ZLIE_ZDOWNE | 
                    LIS3DH_INT1_CFG_YHIE_YUPE | LIS3DH_INT1_CFG_YLIE_YDOWNE |
                    LIS3DH_INT1_CFG_XHIE_XUPE | LIS3DH_INT1_CFG_XLIE_XDOWNE;
...
uint8_t Adafruit_LIS3DH::getReference()
{
    return reference;
}
  
void Adafruit_LIS3DH::setReference(uint8_t ref)
{
    writeRegister8(LIS3DH_REG_REFERENCE, ref);
}
